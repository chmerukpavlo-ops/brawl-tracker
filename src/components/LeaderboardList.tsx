...
interface LeaderboardListProps {
  ...
  isLoading?: boolean;
  highlightedTags?: { mine?: string | null; favorites?: Set<string> };
  rowRefs?: Map<string, HTMLDivElement | null>;
  onSelectPlayer: (ranking: PlayerRanking) => void;
  buildContextMenu?: (ranking: PlayerRanking) => ContextMenuItem[] | undefined;
...
const LeaderboardList = forwardRef<HTMLDivElement, LeaderboardListProps>(
  function LeaderboardList(
    ...
    const myTag = normalizeTag(highlightedTags?.mine ?? null);
    const favorites = highlightedTags?.favorites;
      ...
      return (
        <div ref={ref}>
          <EmptyState
            ...
            title={emptyTitle ?? t("leaders.notFoundInTop")}
          ...
          <section key={group.id} className="space-y-2">
            ...
            <ul className="space-y-2">
              {group.items.map((r) => {
                ...
                const isFav = favorites?.has(norm) ?? false;
                return (
                  <li key={r.tag}>
                    <LeaderboardRow
                      ref={(el) => {
